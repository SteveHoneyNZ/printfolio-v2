 ...
 *
 * @package  WC_Printfolio/Admin/Customers
 * @category Class
 * @author   SteveHoney
 */
class WC_Printfolio_Admin_Customers {
	...
	public function customer_fields( $user ) {
		...
		if ( is_object( $user ) ) {
			$_collections = get_user_meta( $user->ID, '_WC_Printfolio_collections', true );
	...
	public function save_customer_fields( $user_id ) {
		...
		if ( isset( $_POST['collections'] ) ) {
			...
			$old_collections = get_user_meta( $user_id, '_WC_Printfolio_collections', true );
			...
			update_user_meta( $user_id, '_WC_Printfolio_collections', $collections );
		} else {
			update_user_meta( $user_id, '_WC_Printfolio_collections', array() );
		}
	}
...
new WC_Printfolio_Admin_Customers();
