 ...
 *
 * @package  WC_Printfolio/Ajax
 * @category Class
 * @author   SteveHoney
 */
class WC_Printfolio_Ajax {
	...
	public function __construct() {
		// Search collections.
		add_action( 'wp_ajax_WC_Printfolio_search_collections', array( $this, 'search_collections' ) );

		// Batch upload.
		add_action( 'wp_ajax_WC_Printfolio_batch_upload', array( $this, 'batch_upload' ) );

		// Delete image.
		add_action( 'wp_ajax_WC_Printfolio_delete_image', array( $this, 'delete_image' ) );

		// Save images.
		add_action( 'wp_ajax_WC_Printfolio_save_images', array( $this, 'save_images' ) );

		// Add Collection.
		add_action( 'wp_ajax_WC_Printfolio_add_collection', array( $this, 'add_collection' ) );

		// Change collection visibility on my account page.
		add_action( 'wp_ajax_WC_Printfolio_my_account_edit_visibility', array( $this, 'my_account_edit_visibility' ) );
		add_action( 'wp_ajax_nopriv_WC_Printfolio_my_account_edit_visibility', array( $this, 'my_account_edit_visibility' ) );
	}
	...
	public function search_collections() {
		...
		check_ajax_referer( 'WC_Printfolio_search_collections_nonce', 'security' );
	...
	public function batch_upload() {
		...
		check_ajax_referer( 'WC_Printfolio_batch_upload_nonce', 'security' );
		...
		if ( ! empty( $image_metadata['image_meta']['title'] ) ) {
			...
			$title = sprintf( __( 'Photography #%d from %s', 'printfolio' ), $image_id, $first_collection );
		} else {
			$title = sprintf( __( 'Photography #%d', 'printfolio' ), $image_id );
		}
		...
		do_action( 'WC_Printfolio_batch_upload', $id, $image_id, $sku, $regular_price, $collections );

		$response = apply_filters( 'WC_Printfolio_batch_upload_response', array(
			'id'              => $id,
			'image_id'        => $image_id,
	...
	public function delete_image() {
		check_ajax_referer( 'WC_Printfolio_delete_image_nonce', 'security' );
	...
	public function save_images() {
		check_ajax_referer( 'WC_Printfolio_save_images_nonce', 'security' );
		...
		foreach ( $images as $image_id => $image ) {
			 ...
			 */
			do_action( 'WC_Printfolio_save_image_ajax', $image_id, $image );
		}
	}
	...
	public function add_collection() {
		...
		check_ajax_referer( 'WC_Printfolio_add_collection_nonce', 'security' );
	...
	public function my_account_edit_visibility() {
		...
		check_ajax_referer( 'WC_Printfolio_my_account_edit_visibility_nonce', 'security' );
		...
		if ( isset( $_POST['collection_id'] ) && isset( $_POST['customer'] ) && isset( $_POST['visibility'] ) ) {
			...
			$user_collections = get_user_meta( $user->ID, '_WC_Printfolio_collections', true );
