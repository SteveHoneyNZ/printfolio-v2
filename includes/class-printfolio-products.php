 ...
 *
 * @package  WC_Printfolio/Products
 * @category Class
 * @author   SteveHoney
 */
class WC_Printfolio_Products {
	...
	public function product_type( $types ) {
		$types['photography'] = __( 'Photograph', 'printfolio' );
	...
	public function restrict_access() {
		...
		if ( is_product() ) {
			...
			if ( current_user_can( 'manage_printfolio' ) || ! isset( $post->ID ) ) {
				return;
			}
			...
			if ( $images_collections = get_the_terms( $post->ID, 'images_collections' ) ) {
				...
				foreach ( $images_collections as $key => $collection ) {
					...
					if ( WC_Printfolio_is_collection_public( $collection->term_id ) ) {
						return;
					}
				...
				$user_collections = get_user_meta( $user_id, '_WC_Printfolio_collections', true );
		...
		if ( is_tax( 'images_collections' ) ) {
			...
			// Check if is admin.
			if ( current_user_can( 'manage_printfolio' ) ) {
				return;
			}
	...
	public function collections_template( $template ) {
		if ( is_tax( 'images_collections' ) ) {
			...
			if ( ! $template || ( ! empty( $status_options['template_debug_mode'] ) && current_user_can( 'manage_options' ) ) ) {
				$template = WC_Printfolio::get_templates_path() . $file;
			}
		}
	...
	public function setup_image_sizes() {
		...
		$photography_thumbnail = apply_filters( 'WC_Printfolio_get_image_size_thumbnail', array(
		...
		$photography_lightbox = apply_filters( 'WC_Printfolio_get_image_size_lightbox', array(
	...
	public function add_to_order() {
		if ( is_tax( 'images_collections' ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( __( 'Please choose the quantity of items you wish to add to your order&hellip;', 'printfolio' ), 'error' );
	...
	public function validate_before_add_to_cart( $valid, $product_id ) {
		...
		if ( 'photography' == $product_type->name ) {
			...
			wc_add_notice( __( 'You don\'t have permission to purchase this photo!', 'printfolio' ), 'error' );
			return false;
		}
