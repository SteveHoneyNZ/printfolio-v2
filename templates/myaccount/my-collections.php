...
?>
<h2><?php echo apply_filters( 'woocommerce_photography_my_account_collections_title', __( 'My Collections', 'printfolio' ) ); ?></h2>

<form action="" method="post">
	<table class="shop_table shop_table_responsive my-account-collections">
		<thead>
			<tr>
				<th><span class="nobr"><?php _e( 'Cover image', 'printfolio' ); ?></span></th>
				<th><span class="nobr"><?php _e( 'Title', 'printfolio' ); ?></span></th>
				<th><span class="nobr"><?php _e( 'Number of photos', 'printfolio' ); ?></span></th>
				<th><span class="nobr"><?php _e( 'Visibility', 'printfolio' ); ?></span></th>
				<th>&nbsp;</th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				foreach ( $collections as $collection_id ) :
					...
					?>
				<tr>
					<td class="collection-thumbnail" data-title="<?php _e( 'Cover image', 'printfolio' ); ?>" class="cover-img"><?php
						$image        = '';
						...
						echo $image;
					?></td>
					<td class="collection-title" data-title="<?php _e( 'Title', 'printfolio' ); ?>"><a href="<?php echo $url; ?>"><?php echo esc_html( $collection->name ); ?></a></td>
					<td class="collection-count" data-title="<?php _e( 'Number of photos', 'printfolio' ); ?>"><?php echo intval( $collection->count ); ?></td>
					<td class="collection-visibility" data-title="<?php _e( 'Visibility', 'printfolio' ); ?>">
						<?php if ( $customers_can_change_visibility && $customer_can_change_visibility && $customers_can_change_visibility_col ) : ?>
							...
							<button class="button disabled" type="button" data-collection_id="<?php echo intval( $collection_id ); ?>" disabled="disabled"><?php _e( 'Save', 'printfolio' ); ?></button>
						<?php else : ?>
							<?php echo WC_Printfolio_i18n_collection_visibility( $collection_id, $visibility ); ?>
						<?php endif ?>
					</td>
					<td class="collection-actions order-actions wc-actions"><a class="button" href="<?php echo $url; ?>"><?php _e( 'View Photographs', 'printfolio' ); ?></a></td>
				</tr>
			<?php endforeach; ?>
		</tbody>
	</table>
